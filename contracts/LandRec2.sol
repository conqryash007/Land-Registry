...
contract LandRecord{
    ...
    event plotForBuy(uint plotId, address newowner,uint price,uint times);
    ...
    function buyLand(uint _plotId)public {
        Plot memory aux = Plots[_plotId];
        require(aux.inprocess == false);
        aux.newowner = msg.sender;
        aux.inprocess = true;
        Plots[_plotId] = aux;
        emit plotForBuy(_plotId,aux.newowner,aux.sellingPrice,now);
    }
